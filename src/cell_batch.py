...
import math
class CellBatch:
    def __init__(self, dim, leak=0.1, beta=1., bs=1):
        self.dim = dim
        self.bs = bs
        self.z = np.zeros((bs,dim))
        self.phi_z = np.zeros((bs,dim))
        self.dz_td = np.zeros((bs,dim))
        self.dz_bu = np.zeros((bs,dim))
        self.y_node = np.zeros((bs,self.dim))
    ...
    def connect_b(self, b):
        ...
        self.prev_ev = np.zeros((self.bs ,self.dim)) 
    ...
    def connect_t(self, t):
        ...
        self.e_node = np.zeros((self.bs,self.dim))
        self.mu_node = np.zeros((self.bs,self.dim))
    ...
    def integrate(self):
        ...
        """
        temp = np.zeros((self.bs, self.dim))
        if self.t != None:
            temp += self.dz_td * 0.5
        if self.b != None:
            temp += self.dz_bu * 0.5
